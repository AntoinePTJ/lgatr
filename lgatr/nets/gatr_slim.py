"""Equivariant transformer for 3D Euclidean vectors and scalar data."""
...
def _post_attention_reshape(
    ...
    h_v = out[..., : hidden_v_channels * 3].unflatten(-1, (hidden_v_channels, 3))
    h_s = out[..., hidden_v_channels * 3 :]
...
class Dropout(nn.Module):
    """Dropout for 3D Euclidean vector and scalar features.

    For vector features the same dropout mask is applied to all three components of each vector.
    ...
    def forward(
        ...
        ----------
        vectors
            Euclidean vectors of shape ``(..., v_channels, 3)``.
        scalars
        ...
        -------
        outputs_v
            Euclidean vectors with dropout, same shape as ``vectors``.
        outputs_s
...
class RMSNorm(nn.Module):
    """Joint RMS normalization over 3D Euclidean vector and scalar features.
    ...
    def forward(
        ...
        -------
        outputs_v
            Normalized Euclidean vectors, same shape as ``vectors``.
        outputs_s
            Normalized scalar features, same shape as ``scalars``.
        """
        v_squared_norm = vectors.square().sum(-1)
...
class Linear(nn.Module):
    """Linear layer for 3D Euclidean vector and scalar features.
    ...
    initialization
        ...
        for attention projections to improve stability), or ``"mup_hidden"`` (μP hidden-layer init:
        bound scales as ``1/fan_in`` instead of ``1/sqrt(fan_in)``).
    """
    ...
    def forward(
        ...
        ----------
        vectors
            Euclidean vectors of shape ``(..., in_v_channels, 3)``.
        scalars
        ...
        -------
        outputs_v
            Euclidean vectors of shape ``(..., out_v_channels, 3)``.
        outputs_s
...
class GatedLinearUnit(nn.Module):
    """Gated linear unit (GLU) for 3D Euclidean vector and scalar features.

    Scalar gates are computed from scalar features; vector gates are computed from Euclidean
    inner products of (transformed) vector features.
    ...
    def _get_inner_product(self, v_gates_1: torch.Tensor, v_gates_2: torch.Tensor) -> torch.Tensor:
        # 0.57735 = 1/sqrt(3) controls the scale, like 1/sqrt(d_k) in attention
        return 0.57735 * (v_gates_1 * v_gates_2).sum(dim=-1, keepdim=True)


class SelfAttention(nn.Module):
    """Self-attention for 3D Euclidean vectors and scalar features.
    ...
    def _pre_attention_reshape(
        ...
        # Euclidean metric is identity: no metric multiplication needed for inner product
...
class MLP(nn.Module):
    """Multi-layer perceptron for 3D Euclidean vector and scalar features.
...
class GATrSlimBlock(nn.Module):
    """A single block of the GATr-slim network.
...
class GATrSlim(nn.Module):
    """GATr-slim network: Euclidean-equivariant transformer on 3D vectors and scalars.

    A slim GATr variant that operates on Euclidean 3-vectors and scalars (no full multivector
    representation). Stacks ``num_blocks`` :class:`GATrSlimBlock` modules between initial and
    ...
    **compile_kwargs
    ...
    def __init__(
        ...
        **compile_kwargs,
        ...
        self.hidden_v_channels = hidden_v_channels
    ...
    def forward(
        ...
        ----------
        vectors
            Euclidean vectors of shape ``(..., items, in_v_channels, 3)``.
        scalars
        ...
        **attn_kwargs
        ...
        -------
        outputs_v
            Euclidean vectors of shape ``(..., items, out_v_channels, 3)``.
        outputs_s
